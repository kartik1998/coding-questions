...
public class Circle_Of_Strings {
    ...
    public static void topSort(ArrayList<ArrayList<Integer>> adj,int s,boolean vis[],Stack<Integer> stack){
        vis[s]=true;
        for(int i:adj.get(s)){
            if(!vis[i]){
    ...
    public static void dfs(ArrayList<ArrayList<Integer>>adj, int s,boolean vis[]){
        vis[s]=true;
        for(int i:adj.get(s)){
            if(!vis[i]){
                dfs(adj,i,vis);
