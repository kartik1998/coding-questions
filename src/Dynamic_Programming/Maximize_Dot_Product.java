...
public class Maximize_Dot_Product {
    ...
    public static BigInteger helper(int A[],int B[],int n,int m,BigInteger dp[][]){
        if(n==0 || m==0){
        ...
        return dp[n][m]=max(helper(A,B,n-1,m,dp),helper(A,B,n-1,m-1,dp).add(BigInteger.valueOf(A[n-1]*B[m-1])));
    }
